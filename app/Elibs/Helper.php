...
class Helper
{
    ...
    {
        return preg_match("/^\+?\d{10,12}$/i", trim($number));
    ...
    {
        return preg_match('/^[a-z0-9A-Z]+[._]?[a-z0-9A-Z]+$/', $string);
    }
    ...
    public static function formatPercent($number, $sub = false) {
        if ($sub) {
            return $number *100 .'%';
        }
        return $number *100;
...
//        $str = preg_replace('/[^\u4E00-\u9FFF]+/', '', $str);
    ...
    static function showContent($string) {
        ...
        if(is_array($string)) {
            if(isset($string['name']) && $string['name'] != '') {
                return $string['name'];
            }else if(isset($string['src']) && $string['src'] != '') {
                return $string['src'];
            }
        }
        return $string;
    ...
    public static function processRangeMoney($time,$spliter=';',$range=true){
        if (is_string($time) && $time) {
            $updated_at_arr = explode($spliter, $time);
            if(!isset($updated_at_arr[1]) && $range==true){
                $updated_at_arr[1] = $updated_at_arr[0];//tìm trong ngày
            ...
            if ($updated_at_arr && isset($updated_at_arr[0]) && isset($updated_at_arr[1])) {
                $moneyStart = trim($updated_at_arr[0]);
                $moneyEnd = trim($updated_at_arr[1]);
                if (is_numeric($moneyStart) && is_numeric($moneyEnd)) {
                    return [
                        'money_start' =>(int)$moneyStart,
                        'money_end' =>(int)$moneyEnd,
                    ];
                }
            }
        }
    ...
    static function welcome($H){

        if($H < 12){

            return "Buổi sáng an lành";
