...
<div class="alert alert-primary alert-styled-left alert-arrow-left alert-component">
    <div  @if(!@$preview)class="seo-region"@endif>
        <div style="margin-right: 15px;font-style: italic"> Tiêu đề (<i id="seo-title-length">0</i>/70)</div>
        <div placeholder="Tiêu đề seo ở đây, nên có từ khóa" class="seo-title" id="seo-title"  @if(!@$preview)contenteditable="true"@endif>{!! isset($SEO_INPUT_SETTING['TITLE']) && $SEO_INPUT_SETTING['TITLE'] ?$SEO_INPUT_SETTING['TITLE']:'' !!}</div>
        ...
        <div placeholder="Mô tả ngắn nhập ở đây, nên có chứa từ khóa" class="seo-des" id="seo-des"  @if(!@$preview)contenteditable="true"@endif>{!! isset($SEO_INPUT_SETTING['DES'])&& $SEO_INPUT_SETTING['DES']?$SEO_INPUT_SETTING['DES']:'' !!}</div>
...
<div class="col-12">
    @include('forms/input-group-text',[
        'placeholder'=>'Thời trang nữ, thời trang nữ đẹp, đồ nữ đẹp',
        'field'=>['label'=>'Từ khóa khác', 'key'=>'SEO[KEYWORD]', 'name' => 'SEO[KEYWORD]', 'value' => isset($SEO_INPUT_SETTING['KEYWORD'])?$SEO_INPUT_SETTING['KEYWORD']:''],
    ])
    @include('forms/input-group-text',[
        'placeholder'=>'Nội dung thẻ H1: ví dụ Thời trang nữ',
        'field'=>['label'=>'Thẻ H1', 'key'=>'SEO[KEYWORD]', 'name' => 'SEO[H1]', 'value' => isset($SEO_INPUT_SETTING['H1'])?$SEO_INPUT_SETTING['H1']:''],
    ])
    @if(!@$preview)
    ...
    @else
        <div class="form-group">
            <label class="col-form-label control-label">Hình ảnh</label>
            <div class="text-primary">
                {{isset($SEO_INPUT_SETTING['IMAGE'])?$SEO_INPUT_SETTING['IMAGE']:'Chưa cập nhật' }}
            </div>
        </div>
        <div class="form-group" >
            <label class="col-form-label control-label">Robots</label>
            <div class="text-primary">
                {{isset($SEO_INPUT_SETTING['ROBOTS'])?$SEO_INPUT_SETTING['ROBOTS']:'Chưa cập nhật' }}
            </div>
        </div>
    @endif
